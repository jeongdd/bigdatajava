...
public class Reduce02 extends Reducer<Text, Text, Text, IntWritable> {
	...
	private void multiCount(String id, Iterable<Text> values)
	...
	public void reduce(Text key, Iterable<Text> values, Context ctx)
		...
		if (MultipleId[0].equals(cmd[0])) {
			multiCount(MultipleId[0],values); 
		}else if(MultipleId[1].equals(cmd[0])) {
			multiCount(MultipleId[1], values);
		}
	}
